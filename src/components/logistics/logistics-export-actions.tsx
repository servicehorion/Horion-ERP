...
import { exportLogisticsCSV, exportLogisticsPDF } from "@/lib/actions/logistics.actions";

export function LogisticsExportActions() {
  const [exporting, setExporting] = useState<"csv" | "pdf" | null>(null);

  async function handleCsv() {
    setExporting("csv");
    try {
      const res = await exportLogisticsCSV();
  ...
  async function handlePdf() {
    setExporting("pdf");
    try {
      const res = await exportLogisticsPDF();
      if (res.error || !res.data) {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = res.filename || `logistics-${new Date().toISOString().slice(0, 10)}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
    } catch {
